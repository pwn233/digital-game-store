...
class GetProductController(
    ...
    @GetMapping("/{storeBranchId}$URI_PRODUCT")
        ...
        fun getProduct(
            ...
            println(payload)
            val response = facade.process(ProductRequest(storeBranchId, name, price, description, category, brand, barcode,
                pageNo, pageSize))
            println(response+", process time : "+ processTimeDuration(requestProcessTime))
            return response
        }
}
